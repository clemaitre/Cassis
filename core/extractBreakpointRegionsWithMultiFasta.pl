...
# Read the FASTA file and for each fasta entry read the table and create the breakpoint sequences
readMultiFastaAndCreateAllBreakpointFASTAs();
...
Usage:
  perl extractBreakpointRegionsWithMultiFasta.pl <table> <fasta> <outputdir> <genome>
...
Mandatory parameters:
  ...
  <fasta> Path for the file that has the DNA sequence of the whole genome (multiFasta).
...
File formats:
  ...
  The FASTA file must have all chromosome sequences of the genome (multifasta)
...
sub validateParameters {
  ...
  my $fasta     = fileExists(trim($parameters[1]), \&printUsage);
  my $outputdir = directoryExists(trim($parameters[2]), \&printUsage);
  my $genome    = trim($parameters[3]);
...
sub readMultiFastaAndCreateAllBreakpointFASTAs {
...
sub createBreakpointFASTAs {
  ...
  while (my $line = <IN>) {
	...
	if ($chr eq $chrGR) {
	  ...
	  writeFASTA($sequence, $name, $fileName, $columns[$SR_INF], $columns[$SR_SUP]);
	}
	...
	if ($chr eq $chrA) {
	  ...
	  my $fileName = "${outputDir}/${name}.fasta";
	  writeFASTA($sequence, $name, $fileName, $columns[$SA_INF], $columns[$SA_SUP]);
	}
	...
	if ($chr eq $chrB) {
	  ...
	  my $fileName = "${outputDir}/${name}.fasta";
	  writeFASTA($sequence, $name, $fileName, $columns[$SB_INF], $columns[$SB_SUP]);
	}
      }
    }
  }
...
sub writeFASTA {
  my ($sequence, $name, $file, $begin, $end) = @_;
