...
Usage:
  perl createBreakpointFastas.pl <table> <chrGR> <chrGO> <outputdir>
...
Mandatory parameters:
  ...
  <chrGR> Path for the file that has information about the chromosomes of the
          genome GR.

  <chrGO> Path for the file that has information about the chromosomes of the
          genome GO.
...
File formats:
  ...
  The chromosome information file (parameters <chrGR> and <chrGO>) is a TSV
  (TAB SEPARATED VALUES) file each contains the following columns:

  + chr   - Name of the chromosome
  + begin - Start position of the chromosome sequence (usually 1)
  + end   - End position of the chromosome sequence (sequence length)
  + file  - Path for the FASTA file that has the sequence

  This file is produced by the script verifyFASTAdirectory.pl
...
sub validateParameters {
  ...
  my $chrGR     = fileExists(trim($parameters[1]), \&printUsage);
  my $chrGO     = fileExists(trim($parameters[2]), \&printUsage);
  my $outputdir = directoryExists(trim($parameters[3]), \&printUsage);

  $PARAMETERS{"TABLE"}     = $table;
  $PARAMETERS{"CHRGR"}     = $chrGR;
  $PARAMETERS{"CHRGO"}     = $chrGO;
...
sub verifyEnvironment {
  my $corescript = "${Bin}/../core/extractBreakpointRegions.pl";
  if (! -e $corescript) {
...
sub createSRfastas {
  ...
  my $chrGR     = $PARAMETERS{"CHRGR"};
  my $outputdir = $PARAMETERS{"OUTPUTDIR"};
  my $script    = $ENVIRONMENT{"CORESCRIPT"};

  open(IN, "${chrGR}") or die printUsage("Could not read the file ${chrGR}");
  while (my $line = <IN>) {
    chomp($line);
    my ($chr, $begin, $end, $file) = split("\t", $line);
    my $cmd = "perl ${script} ${table} ${chr} ${file} ${outputdir} 1";
    print "Creating sequences SR from chromosome $chr (Genome GR)\n";
    system("$cmd");
  }
  close(IN);
...
sub createSABfastas {
  ...
  my $table     = $PARAMETERS{"TABLE"};
  my $chrGO     = $PARAMETERS{"CHRGO"};
  my $outputdir = $PARAMETERS{"OUTPUTDIR"};
  my $script    = $ENVIRONMENT{"CORESCRIPT"};

  open(IN, "${chrGO}") or die printUsage("Could not read the file ${chrGO}");
  while (my $line = <IN>) {
    chomp($line);
    my ($chr, $begin, $end, $file) = split("\t", $line);
    my $cmd = "perl ${script} ${table} ${chr} ${file} ${outputdir} 2";
    print "Creating sequences SA and SB from chromosome $chr (Genome GO)\n";
    system("$cmd");
  }
  close(IN);
